...
export class CategoryService {
            ...
            fs.access("./" + imageUrl,
                (err) => {
                    ...
                    if (!mainCat.subCategories)
                        ...
                        this.enhanceSubCategoryData(mainCat.name, mainCat.subCategories)
    ...
    getSubCategories(mainCategoryName: string): PromiseLike<ISubCategory[]> {
        return Promise<ISubCategory[]>((resolve, reject) => {
            this.getCategoryData()
                .then(categoryData => {
                    var mainCat = _(categoryData).find((cat) => { return cat.name == mainCategoryName });
                    if (mainCat)
                        resolve(mainCat.subCategories);
                    else
                        reject("Not found");
                },
                err => {
     ...
     */
    getUsedMainCategories(): PromiseLike<IMainCategory[]> {
     ...
     */
    getUsedSubCategories(mainCategoryName: string): PromiseLike<ISubCategory[]> {
        var dummyMainCat = <IMainCategory>{};
        dummyMainCat.name = mainCategoryName;
        ...
        return proposalModel.Proposal.aggregate(
            ...
            .then(subCategories=> {
                ...
                // Enhance the data with image URLs
                return this.enhanceSubCategoryData(dummyMainCat, <ISubCategory[]><any>subCategories);
            });
